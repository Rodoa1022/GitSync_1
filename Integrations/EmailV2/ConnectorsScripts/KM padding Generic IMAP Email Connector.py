...
from DEBUG_EmailIMAPManager import EmailIMAPManager
    ...
    def run(self):
        ...
        try:
            ...
            #last_calculated_run_time = get_last_success_time(siemplify=self.connector_scope,
                                                             #offset_with_metric={'days': self.offset_in_days}) - timedelta(minutes=177)
            
            #last_calculated_run_time = get_last_success_time(siemplify=self.connector_scope,
                                                             #offset_with_metric={'days': self.offset_in_days}) 
            last_calculated_run_time = datetime.now() - timedelta(days=self.offset_in_days)
