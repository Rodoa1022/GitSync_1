...
class EmailModelBuilder(BaseEmailBuilder):
    ...
    def _extract_subject(self, msg):
        ...
        try:
            final_result = []
            for parsed_value, encoding in decode_header(raw_subject):
                if isinstance(parsed_value, bytes):
                    subject = (
                        parsed_value.decode()
                        if encoding is None else parsed_value.decode(encoding)
                    )
                else:
                    subject = parsed_value
                regex_object = re.compile(
                    self.regex_map.get('subject'),
                    re.MULTILINE
                )
            ...
            self.logger.error(
                'Unable to extract subject from email with '
                f'email_uid={self.email.email_uid}'
            )
            self.logger.exception(e)
